...
export class LoginComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.oauthService.getOAuthProviders().subscribe({
      next: (response) => {
        console.log('OAuth providers response:', response);
        this.oauthProviders.set(response.providers);
        console.log('OAuth providers set to:', this.oauthProviders());
      },
      error: (error) => {
        console.error('Failed to load OAuth providers:', error);
        // Set default values if request fails
        this.oauthProviders.set({ google: true, facebook: false, apple: false });
      }
    });
  }
